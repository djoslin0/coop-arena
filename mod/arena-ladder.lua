...
function bhv_arena_ladder_init(obj)
    obj.hitboxRadius = 40
    obj.hitboxHeight = math.floor(obj.oBehParams*.1)
    obj.oFaceAngleYaw = (obj.oBehParams-math.floor(obj.oBehParams*.1)*10)*16384-32768
end
...
function act_ladder(m)
    ...
    if m.pos.y > ladder.oPosY or m.pos.y < ladder.oPosY + ladder.hitboxHeight then
        climb = climb + math.abs(m.controller.rawStickY*.2)
    end
    if climb > 128 then
        climb = 0
    ...
    if (m.input & (INPUT_A_PRESSED | INPUT_B_PRESSED)) ~= 0 then
        ...
        m.vel.y = m.controller.rawStickY*.2
    end
end
hook_event(HOOK_MARIO_UPDATE, formario)
